# 메이플스토리 유저 분석

넥슨 오픈API로 수집한 실제 유저 데이터를 바탕으로 경험치 변동 패턴과 유저 행동을 분석합니다.
동일한 수집 데이터를 공유하지만 분석 방향이 다른 **독립된 두 프로젝트**로 구성되어 있습니다.

> **수집 기간 2025.11.08 ~ 2026.03.14 (126일) · 고정 스냅샷**
> 해당 기간에 수집을 마친 데이터를 분석한 결과이며, 이후 데이터는 갱신하지 않습니다.
> 대시보드는 계속 접속할 수 있으나 표시되는 수치는 위 기간 기준으로 고정되어 있습니다.

## I. 경험치 분석 (`exp_analysis/`)

- [대시보드 바로가기](https://maple-exp.streamlit.app/)
- [분석 리포트](https://www.notion.so/miniminimin/32afbcdaed288075a929eb7f533361b0?source=copy_link)

쇼케이스·이벤트·요일 등 외부 요인이 고레벨 유저(Lv.285~299)의 사냥 동기에 미치는 영향을 검증합니다.

### 1. 쇼케이스 영향 분석

- 쇼케이스 기준일(2025.12.13) ±35일 완전 대칭 구간으로 Pre/Post 비교
- 레벨 구간별(Lv.285-289 / 290-294 / 295-299) 일평균 경험치 변화율 제공
- 대응표본 t-검정으로 구간별 유의성 검증

### 2. 주간 패턴 분석

- 수요일 시작 7일 단위 요일별 평균 경험치 분포 제공
- 주차 × 요일 히트맵으로 시기별 사냥 패턴 시각화
- 메요일(목) / 썬데이(일) 강조 표시

### 3. 썬데이 이벤트 심층 분석

- 썬데이 이벤트를 경타포스 / 사냥 / 사냥 외 3분류로 구분
- 유저별 Pre 평균 대비 상대 경험치 비율로 레벨 구간 간 기저 차이 제거
- One-way ANOVA + Tukey HSD 사후 검정으로 이벤트 유형 간 유의성 검증

### 4. 이벤트 기간 영향 분석

- `eventlog.txt`에 등록된 이벤트에 대해 영향 집계
- 구간별 대응표본 t-검정 결과 제공
- 쇼케이스 이후 일별 추이에 이벤트 기간 하이라이트

---

## II. 유저 클러스터링 (`user_clustering/`)

- [대시보드 바로가기](https://maple-user-clustering.streamlit.app/)
- [분석 리포트](https://www.notion.so/miniminimin/346fbcdaed2880209e39ff1b490e34e5?source=copy_link)


활동 패턴·성장 효율·캐릭터 연령 등 행동 지표를 기반으로 유저를 군집화하고, 이탈 패턴과 스펙 분포를 분석합니다.

### 1. 유저 이탈 분석

- 7일 연속 비활성을 이탈로 정의한 Kaplan-Meier 생존 분석 (Lv.285+ 고레벨 유저는 일상적 사냥 패턴이 규칙적이므로 7일 기준 적용)
- 레벨 구간별·클러스터별 이탈 곡선 및 중앙 이탈 시점 비교
- 다중·쌍별 로그랭크 검정으로 그룹 간 생존 분포 유의성 검증

### 2. 클러스터 분석

- 활동 일수 비율 / 경험치 퍼센타일 / 유니온 레벨 / 캐릭터 연령 4개 피처로 클러스터링
- KMeans·GMM 양 모델 실루엣 점수 비교 후 최적 (모델, k) 자동 선택 (k=2~7 탐색)
- 세그먼트 × 클러스터 교차 분포, 쇼케이스 전·후 대칭 기간 경험치 변화 비교
- ANOVA 로 클러스터 간 피처 차이 유의성 검증, 자동 레이블 힌트 출력

### 3. 스펙 분포 분석

- 레벨 구간·월드 그룹별 능력치 바이올린 플롯
- tier × world_group 전투력 중앙값 히트맵

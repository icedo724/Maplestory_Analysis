...
from config import SHOWCASE_DATE, AGG_DIR
from loader import load_agg

st.set_page_config(page_title="메이플스토리 쇼케이스 분석", layout="wide")


def main():
    if not __import__('os').path.exists(AGG_DIR):
    ...
    with tab1:
        ...
        st.info(f"**대칭 분석 기간:** {sym_start.strftime('%Y-%m-%d')} ~ {sym_end.strftime('%Y-%m-%d')}"
                f"  (±{sym_days}일)  |  전체 데이터: {data_start} ~ {data_end}")
        ...
        with col1:
            ...
            trend = df_sym.groupby('date')['avg_exp'].mean().reset_index()
            fig   = px.line(trend, x='date', y='avg_exp', markers=True,
                            labels={'avg_exp': '평균 일일 경험치', 'date': '날짜'})
            fig.add_vline(x=SHOWCASE_DATE, line_width=2, line_dash="dash", line_color="red")
            fig.add_annotation(x=SHOWCASE_DATE, y=1, yref="paper",
        ...
        with col2:
            ...
            df2 = df_sym.copy()
            df2['baseline']  = df2['segment'].map(baseline)
            df2['Exp_Ratio'] = df2['avg_exp'] / df2['baseline'] * 100
            fig2 = px.line(df2, x='date', y='Exp_Ratio', color='segment', markers=True,
                           labels={'Exp_Ratio': '평상시 대비 (%)', 'date': '날짜'})
            fig2.add_hline(y=100, line_dash="dot", line_color="gray")
            fig2.add_vline(x=SHOWCASE_DATE, line_width=2, line_dash="dash", line_color="red")
            fig2.add_annotation(x=SHOWCASE_DATE, y=1, yref="paper",
                                text="Showcase", showarrow=False, font=dict(color="red", size=13))
        ...
        with c_bar:
            fig_bar = px.bar(summary, x='segment', y='growth_rate', color='segment',
                             title=f"구간별 성장 증감률 (%) — ±{sym_days}일 기준",
                             text='growth_rate',
    ...
    with tab2:
        ...
        with col_s1:
            ...
            sel_w     = st.selectbox("주차 선택:", ["전체 주차 (평균)"] + [f"{w}주차" for w in all_weeks])
        with col_s2:
        ...
        fig_week.add_annotation(x=1, y=y_m * 0.1, text="목(메요일)", showarrow=False, font=dict(color="orange"), textangle=-90)
        fig_week.add_vline(x=4, line_width=1.5, line_dash="dot", line_color="green")
        fig_week.add_annotation(x=4, y=y_m * 0.1, text="일(썬데이)", showarrow=False, font=dict(color="green"), textangle=-90)
        ...
        pivot = hm_df.pivot_table(index='week_idx', columns='day_name', values='avg_exp', aggfunc='mean')
        ...
        sc_week_idx = int(((showcase_dt - pd.Timedelta(days=2) -
                            (pd.to_datetime(daily['date'].min()) - pd.Timedelta(days=2))).days) // 7) + 1
        sc_label = f"{sc_week_idx}주차"
        if sc_label in pivot.index:
            fig_heat.add_hline(y=list(pivot.index).index(sc_label), line_dash="dash", line_color="red",
                               line_width=1.5, annotation_text="쇼케이스", annotation_position="right")
        st.plotly_chart(fig_heat, use_container_width=True)

    # ── TAB 3 ────────────────────────────────────────────────────────────
    with tab3:
        st.markdown(
            "썬데이 메이플 종류에 따라 유저들의 **사냥 동기 변화율**이 달라지는지 검증합니다.  \n"
            "절댓값 경험치가 아닌 **유저별 평상시 대비 해당 썬데이의 변화율**을 기준으로 분석합니다."
        )
        ...
        with col_sub1:
            st.info("썬데이 분류 현황")
            log_display = sun_ev[['Date', 'Sunday_Type', 'Event_Category']].drop_duplicates().sort_values('Date')
            ...
            sel_seg_sun = st.selectbox("분석 그룹:",
                                       ["전체 유저"] + sorted(sun_box['scope'].unique().tolist()), key='sun_seg')
        ...
        with col_sub2:
            ...
            for i, cat in enumerate(cat_order):
                ...
                fig_box.add_trace(go.Box(
                    name=cat, q1=[r['p25']], median=[r['median']], q3=[r['p75']],
                    lowerfence=[r['p5']], upperfence=[r['p95']], mean=[r['mean']],
                    boxmean=True, marker_color=colors[i], x=[cat],
                ))
            fig_box.add_hline(y=100, line_dash="dot", line_color="gray",
                              annotation_text="평상시 기준(100%)", annotation_position="right")
            fig_box.update_layout(title=f"[{sel_seg_sun}] 이벤트 유형별 변화율 (상위 1% 제외)",
                                  yaxis_title='평상시 대비 (%)', showlegend=False)
        ...
        fig_mean = px.bar(box_all, x='category', y='mean', color='category', text='mean',
        ...
        st.caption("  |  ".join([f"**{r['category']}** n={int(r['n']):,}" for _, r in box_all.iterrows()]))

        st.divider()
        st.subheader("ANOVA 검정")
        anova_row = anova[anova['scope'] == scope_key]
        if not anova_row.empty:
            r          = anova_row.iloc[0]
            mean_cols  = {c.replace('mean_', ''): r[c] for c in r.index if c.startswith('mean_')}
            means_text = " | ".join([f"**{k}**: {v:.1f}%" for k, v in mean_cols.items()])
            if r['p_value'] < 0.05:
                st.success(f"**유의미한 차이 존재 (F={r['f_stat']:.2f}, P={r['p_value']:.4e})**\n\n*(평균 변화율: {means_text})*")
            else:
                st.info(f"**유의미한 차이 없음 (F={r['f_stat']:.2f}, P={r['p_value']:.4f})**\n\n*(평균 변화율: {means_text})*")

            if not tukey.empty:
                st.markdown("#### 사후 검정 (Tukey HSD)")
                tukey_scope = tukey[tukey['scope'] == scope_key].copy()
                tukey_scope['유의미'] = tukey_scope['reject'].apply(lambda x: "차이 있음" if x else "차이 없음")
                st.dataframe(tukey_scope[['group1', 'group2', 'meandiff', 'p-adj', '유의미']], use_container_width=True)

    # ── TAB 4 ────────────────────────────────────────────────────────────
    with tab4:
        st.markdown("특정 이벤트 구간과 비이벤트 구간을 비교하여 추가 효과를 검증합니다.")
        if event_impact.empty:
            st.info("이벤트 영향 분석 데이터가 없습니다.")
        else:
            events_list = event_impact[['event_name', 'event_type', 'event_start', 'event_end']].drop_duplicates()
            ...
            st.info(f"**{ev_meta['event_name']}** ({ev_meta['event_start']} ~ {ev_meta['event_end']})  |  유형: {ev_meta['event_type']}")

            col_e1, col_e2 = st.columns(2)
            with col_e1:
                fig_ev = px.bar(ev_data, x='segment', y='growth_rate', color='segment', text='growth_rate',
                                title="이벤트 vs 비이벤트 기간 변화율 (%)",
            ...
            with col_e2:
                ev_melt = ev_data.melt(id_vars='segment', value_vars=['non_event_avg', 'event_avg'],
                                       var_name='구간', value_name='평균 경험치')
                ev_melt['구간'] = ev_melt['구간'].map({'non_event_avg': '비이벤트', 'event_avg': '이벤트'})
                fig_cmp = px.bar(ev_melt, x='segment', y='평균 경험치', color='구간', barmode='group',
                                 title="비이벤트 vs 이벤트 기간 평균 경험치",
                                 labels={'segment': '레벨 구간'})
                st.plotly_chart(fig_cmp, use_container_width=True)

            st.subheader("쇼케이스 이후 일별 경험치 추이")
            post_daily = daily[daily['date'] > pd.to_datetime(SHOWCASE_DATE)]
            trend_post = post_daily.groupby('date')['avg_exp'].mean().reset_index()
            fig_trend  = px.line(trend_post, x='date', y='avg_exp', markers=False,
                                 labels={'avg_exp': '평균 일일 경험치', 'date': '날짜'})
            fig_trend.add_vrect(x0=ev_meta['event_start'], x1=ev_meta['event_end'],
            ...
            for _, row in ev_data.iterrows():
                p         = row['p_value']
                sig       = "매우 유의미 (***)" if p < 0.001 else "유의미 (*)" if p < 0.05 else "차이 없음"
                direction = "증가" if row['event_avg'] > row['non_event_avg'] else "감소"
                st.info(f"**[{row['segment']}]** n={int(row['n']):,}\n"
                        f"* 변화: {direction} ({row['growth_rate']:+.1f}%)  ({sig})\n"
                        f"* P-value: {p:.4e}")
